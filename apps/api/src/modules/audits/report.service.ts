...
<style>
  * { box-sizing: border-box; }
  ...
  .section h2 { font-size: 18px; border-left: 4px solid #2563EB; padding-left: 10px; margin-bottom: 12px; }
  ...
  .rec-num { width: 24px; height: 24px; border-radius: 50%; background: #2563EB; color: #fff; font-size: 12px; display: flex; align-items: center; justify-content: center; }
...
<body>
  ...
  private scoreColor(v: number): string {
    if (v >= 90) return '#22c55e'; // excellent
    if (v >= 70) return '#4F8CFF'; // good
    if (v >= 40) return '#F59E0B'; // needs improvement
