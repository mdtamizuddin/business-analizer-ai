...
export function RecommendationCard({ rec }: { rec: AuditRecommendation }) {
  ...
  return (
    <Card className="hover:border-primary/50 hover:shadow-glow-primary transition-all">
    <CardHeader className="flex flex-row items-center justify-between">
      <div className="flex items-center gap-2">
        <Badge variant="info">{TYPE_LABELS[rec.type] ?? rec.type}</Badge>
          <span
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2 text-sm">
          <div>
            ...
            <span className="text-slate-300">{rec.problem}</span>
          </div>
          <div>
          ...
          <div>
            <span className="font-medium text-text-secondary">Business Impact: </span>
            <span className="text-slate-300">{rec.businessImpact}</span>
          </div>
        </div>
        <div className="flex gap-4 pt-2 border-t border-border text-xs text-text-secondary">
          <span>Effort: <span className="font-medium text-text-primary">{rec.estimatedEffort}</span></span>
          <span>ROI: <span className="font-medium text-text-primary">{rec.estimatedRoi}</span></span>
        </div>
      </CardContent>
    </Card>
  );
